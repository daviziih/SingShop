...
public class Utilidades {
	...
	public static boolean isLojaValid(String[] lines) {
		...
		if ((!price.matches("^c[1-9](\\d)*:v[1-9](\\d)*(\\s|$)")) && (!price.matches("^c[1-9](\\d)*(\\s|$)"))
	...
	public static String updatePriceSign(String line) {
		if (replace(line).matches("^(?i)c([1-9]){1}(\\d)*:(?i)v([1-9]){1}(\\d)*")) {
			return "§2§lC§r " + replace(line).split(":")[0].replace("C", "").replace("c", "") + " : §4§lV§r "
					+ replace(line).split(":")[1].replace("V", "").replace("v", "");
		}
	...
	public static boolean haveSlotClearInv(Inventory inventory, ItemStack itemStack, int amount) {
		...
		for (int i = 0; i < inventory.getSize(); i++) {
			...
			} else if ((itemStack.isSimilar(item)) && (itemStack.getMaxStackSize() != 1)) {
	...
	public static int quantidadeItemInventory1(Inventory inventory, ItemStack itemStack) {
		int quantia = 1;
		for (ItemStack item : inventory.getContents()) {
			if ((item != null) && (item.isSimilar(itemStack))) {
				quantia -= item.getAmount();
